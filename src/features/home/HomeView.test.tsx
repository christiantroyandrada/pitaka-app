...
describe('HomeView', () => {
  it('shows the balance', async () => {
    ...
    await render(
    <HomeView balanceCentavos={645613} grid={grid} flags={{}} onNavigate={onNavigate} />
    )
  ...
  it('renders the enabled tiles', async () => {
    const onNavigate = jest.fn()
    await render(
      <HomeView balanceCentavos={0} grid={grid} flags={{}} onNavigate={onNavigate} />
    )
  ...
  it('does not render a tile disabled by greylisting flag', async () => {
    ...
    await render(
      <HomeView balanceCentavos={0} grid={grid} flags={flag} onNavigate={onNavigate} />
    )
