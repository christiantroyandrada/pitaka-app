...
Native also checks `(method, path)` against a compile-time allowlist of H5-reachable routes and rejects the rest with `UNAUTHORIZED`. Money movement isn't on that list: `payments.requestPayment` is performed by native behind its own confirmation sheet. Same rule as `registryKey` in `src/config/schema.ts`. Bound untrusted input at the boundary, not at the call site.
