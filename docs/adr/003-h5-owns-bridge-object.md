...
The client is pinned into each H5 bundle (`pitaka-bridge#vX.Y.Z`), so a client-side fix means every H5 bumps and redeploys. Native can't hot-patch the web half. Every load pays the probe.
...
- A deployed H5 needs a bridge fix and can't be redeployed. That is the cost this ADR accepts; one occurrence is enough to revisit.
