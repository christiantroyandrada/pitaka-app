...
`src/domain/ledger.ts` stores `LedgerEntry` records and nothing else. `balanceOf` derives a balance by summing entries for an account; there's no stored balance anywhere, including in `walletStore`, which calls `balanceOf` on read.
...
Balances cost O(entries) per read. That is fine at demo scale and wouldn't be at production scale, where the answer is a periodic snapshot plus entries since, never a mutable column. Callers must pass centavos, so every UI boundary needs an explicit parse, and `parseAmountToCentavos` returning `null` has to be handled rather than coerced.
