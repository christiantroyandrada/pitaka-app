...
`parseServicesGrid(undefined)` in `src/app/home.tsx` with a real fetch. Everything
downstream — the schema, the fallback, the resolver, the fixture matrix — already
exists and is tested. Needs `expo prebuild` and a dev client, so this is the step
...
- **Typed routes only know files that exist.** `router.push('/x')` fails typecheck until
  ...
  `expo export`, and `.expo/` is gitignored — so **CI cannot catch a bad route path.**
...
- **A monorepo for the H5 apps.** The separate repos are the point —
